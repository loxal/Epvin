...
//        }
  ...
  private void fetchAndEdit() {
    ...
    Request<EmployeeProxy> fetchRequest = cf.getRf().find((EntityProxyId<EmployeeProxy>) employee.stableId());

    fetchRequest.with(Driver.DRIVER.getPaths());

    fetchRequest.to(new Receiver<EmployeeProxy>() {
      @Override
      public void onSuccess(EmployeeProxy person) {
